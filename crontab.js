...
if (process.env.CRON_PATH !== undefined) {
  console.log(
    `Path to crond files set using env variables ${process.env.CRON_PATH}`
  );
...
exports.update = function(data) {
  db.findOne({ name: data.name }, function(err, doc) {
    if (!doc || doc === []) {
      var tab = crontab(
        data.name,
        data.command,
  ...
  db.find({})
    ...
    .exec(function(err, docs) {
      for (var i = 0; i < docs.length; i++) {
        if (docs[i].schedule == "@reboot") docs[i].next = "Next Reboot";
        else
          docs[i].next = cron_parser
            .parseExpression(docs[i].schedule)
            .next()
            .toString();
      }
      callback(docs);
  ...
  exports.crontabs(function(tabs) {
    ...
    tabs.forEach(function(tab) {
      if (!tab.stopped) {
        ...
        if (tab.command[tab.command.length - 1] != ";")
          // add semicolon
          tab.command += ";";
    ...
    fs.writeFile(exports.env_file, env_vars, function(err) {
      ...
      fs.writeFile(path.join(cronPath, fileName), crontab_string, function(
        err
      ) {
...
exports.backup = function() {
  ...
  fs.createReadStream(__dirname + "/crontabs/crontab.db").pipe(
...
exports.restore = function(db_name) {
  fs.createReadStream(__dirname + "/crontabs/" + db_name).pipe(
    fs.createWriteStream(__dirname + "/crontabs/crontab.db")
  );
  ...
  exec("crontab -l", function(error, stdout, stderr) {
    ...
    lines.forEach(function(line, index) {
      ...
      if (command && schedule && is_valid) {
        ...
        db.findOne({ command: command, schedule: schedule }, function(
          err,
          doc
        ) {
          if (err) {
